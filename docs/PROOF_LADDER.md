...
| 2 | Contract unit tests | **PARTIAL** | 9 tests on `CovenantStatusLib`. The covenant contract does not exist yet, so this rung is reached for the state machine only |
| 3 | Fuzz and invariant tests | **PARTIAL** | 3 fuzz tests at 512 runs, 3 invariants at 256 runs × depth 64 (16,384 calls, 0 reverts). Same caveat: model only |
