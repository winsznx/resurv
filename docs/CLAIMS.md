...
| The deployed contracts are built from current `main` and contain both audit rounds' fixes | VERIFIED | Redeployed 2026-08-13 from commit `b9f8722` under the `resurv/v4` salt namespace. `git diff b9f8722 -- packages/contracts/` is empty, so the deployed source is exactly that commit's. All six predicted addresses matched. Deployed runtime bytecode compared byte for byte against local artifacts: three exact, three equal-length differing only in immutable slots whose decoded values are the recorded addresses. Was `REFUTED` until this redeployment | 2026-08-13 | Contracts |
...
| Five of the six deployed contracts verify on Sourcify at `match` level | VERIFIED | Creation **and** runtime bytecode reproduce from commit `b9f8722`, propagated to Blockscout. Reproduce with `curl -s https://sourcify.dev/server/v2/contract/84532/<address>`. **`ResurvCovenantManager` is not verified**: three submissions, including from a clean build, answered `no_match`. The other five verify from the same build with the same settings, so the cause is specific to that contract and is unresolved. Its bytecode is still checkable with `cast code` against the local artifact, which is what `docs/DEPLOYMENTS.md` documents | 2026-08-13 | Contracts |
