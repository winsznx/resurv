...
| A funded covenant always has a callable exit | VERIFIED (local EVM) | `test_aFundedCovenantAlwaysHasAnExit`, plus `test_aSatisfiedCovenantCanStillBeClosedAfterItsDeadline`, `test_aCodelessVerifierDoesNotTrapTheEscrow` and `test_aVerifierWithShortReturnDataDoesNotTrapTheEscrow`. All four exist because an in-repo `contracts-auditor` review found three ways the escrow could be trapped permanently, with working proofs. Fixed and redeployed. `docs/phase-logs/PHASE_06_REVIEW.md` | 2026-08-12 | Contracts |
| An expiry refund is impossible while the verifier answers true, whatever gas the caller supplies | VERIFIED (local EVM) | `test_expiryRefusesATrueOutcomeEvenWhenTheCallerStarvesTheVerifier` runs the same covenant at 60M and 2M gas and asserts opposite outcomes. PRD invariant 10.14.8 | 2026-08-12 | Contracts |
...
| The review findings are closed and regression-tested | VERIFIED (local EVM) | `packages/contracts/test/AuditRegressions.t.sol`, 12 tests. **The reviewers are in-repo agents with no write access, not a third-party security audit.** RESURV has had no external audit | 2026-08-12 | Contracts |
