...
| 5 | KeeperHub seam tests | not reached from this repository | `packages/seam-probe` builds twelve scenarios covering the whole attempt seam, and has never run: no credential exists here. Built is not reached. Equivalent probes were run from `keeperhub-flightcheck` and are recorded as `MEASURED_EXTERNAL` in `docs/CLAIMS.md`; that is not this rung either |
...
A third thing looks like evidence and is not: an experiment that exists. `packages/seam-probe`
is complete, typechecked and covered by offline tests, and it has produced zero measurements.
Code that would prove something promotes no rung until it has been run and its output committed.
...
Unchanged since Phase 0, and now with the experiment sitting next to it unrun.

Rung 8 requires an atomic attempt where a false outcome reverts the whole thing. Nobody has yet
measured what KeeperHub returns when a broadcast transaction reverts onchain, as opposed to a
simulation predicting a revert. `receiptStatus` has a documented `reverted` value so the outcome
is representable, but whether an execution id and hash come back, what `status` settles to, and
whether the revert reason survives are all unknown. The current documentation is silent on all
three, and Phase 0.5 confirmed that silence rather than assuming it. If a reverted attempt is
indistinguishable from a network failure, the proof page cannot tell the story the product is
built on.

Rung 8 has also acquired a second requirement that Phase 0.5 found by reading rather than by
measuring: a transaction receipt with status `0x1` does not prove the attempt succeeded, because
`safe_inner_failure` is a documented receipt status. Reaching this rung means showing the
expected event, not the receipt. `docs/THREAT_MODEL.md` T15.

One command closes the gap: `pnpm --filter @resurv/seam-probe test:seam`, after a human pastes
the key. It belongs ahead of any contract work.
