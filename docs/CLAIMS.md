...
| The deployed contracts are built from current `main` and contain both audit rounds' fixes | VERIFIED | Redeployed 2026-08-13 from commit `1d1eb9a` under the `resurv/v5` salt namespace. All six runtime bytecode hashes recompute from source and match the manifest, and all six verify independently on Sourcify. All six predicted addresses matched. Was `REFUTED`, then briefly and wrongly `VERIFIED` against a generation whose manager was a mutant | 2026-08-13 | Contracts |
...
| The deployed Worker holds no credential | VERIFIED | `workerEnvSchema` makes `KEEPERHUB_API_KEY` optional and `apps/worker/test/health.test.ts` pins that a bare environment answers `200 ok`. Changed after a review found the Worker required a live write-capable organization key so that one health route could report its presence, with nothing else reading it . Confirmed on the live origin: `curl -s https://resurv-production.timjosh507.workers.dev/api/health` returns `configured: true` from an environment provisioned with no KeeperHub key | 2026-08-13 | Engineering |
...
| Every committed CI job passed on a clean GitHub runner | VERIFIED (three runs) | GitHub Actions runs [31642439279](https://github.com/winsznx/resurv/actions/runs/31642439279) (`f4a9578`), [31653312589](https://github.com/winsznx/resurv/actions/runs/31653312589) (`d0f0e3a`) and [31653764278](https://github.com/winsznx/resurv/actions/runs/31653764278) (`ba30998`, the tip), 2026-08-12 and 2026-08-13. All four jobs green in both: `format, lint, typecheck, test, build`; `forge fmt, build, test, invariant`; `no secrets in the tree, no permission bypass, no script drift`; `repository gate`. Three observations on clean runners, not a guarantee about future dependency or runner changes | 2026-08-13 | Engineering |
| All six deployed contracts verify on Sourcify at `match` level | VERIFIED | Creation **and** runtime bytecode reproduce from commit `1d1eb9a`, propagated to Blockscout. Reproduce with `curl -s https://sourcify.dev/server/v2/contract/84532/<address>`. The previous generation verified only five: independent recompilation refused the sixth because the deployment had read a stale artifact cache and shipped a mutation-testing build of the manager. That is what this row is for | 2026-08-13 | Contracts |
